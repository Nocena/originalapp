...
async function main() {
  try {
    ...
    const video = await fetchBlobFromUrl(
      `/api/proxy-ipfs/route?url=${encodeURIComponent(videoUrl)}`
    );
    const photo = await fetchBlobFromUrl(
      `/api/proxy-ipfs/route?url=${encodeURIComponent(photoUrl)}`
    );
